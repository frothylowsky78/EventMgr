...
/** GET /admin/events/{eventId}/locations — all named places incl. unpublished. */
  ...
  try {
    ...
    const res = await ddb.send(
      new QueryCommand({
        ...
        ExpressionAttributeValues: { ':pk': keys.locationList(eventId).GSI1PK },
      })
    );
    return ok((res.Items ?? []).map(toEventLocation));
  } catch (e) {
    return fail(e);
  }
};
